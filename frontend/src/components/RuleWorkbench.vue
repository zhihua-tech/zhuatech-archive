...
const fields = [{"key":"archiveNo","label":"档案编号","type":"text"},{"key":"retentionExpiry","label":"保管到期日","type":"date"},{"key":"reviewDate","label":"复核日期","type":"date"},{"key":"legalHold","label":"法务冻结","type":"boolean"},{"key":"pendingBorrow","label":"存在未归还借阅","type":"boolean"},{"key":"appraisalApproved","label":"鉴定已批准","type":"boolean"},{"key":"integrityVerified","label":"完整性已验证","type":"boolean"}]
const form = reactive({"archiveNo":"ARC-2020-001","retentionExpiry":"2025-12-31","reviewDate":"2026-08-26","legalHold":false,"pendingBorrow":false,"appraisalApproved":true,"integrityVerified":true})
...
async function run() {
  busy.value = true
  error.value = ''
  try { result.value = await request('/api/enterprise/archive/review-disposal', { method: 'POST', body: JSON.stringify(payload()) }) }
...
<template>
  <section class="rule-head">
    <div><span>DOMAIN CONTROL</span><h3>档案处置合规复核</h3><p>联合检查保管期限、法务冻结、借阅、鉴定审批与完整性，输出销毁准入结论。</p></div>
    <b>服务端强校验</b>
  </section>
